...
//        htmlReporter.config().setResourceCDN(ResourceCDN.GITHUB);
        ...
        htmlReporter.config().setDocumentTitle("crm自动化测试报告");
        htmlReporter.config().setReportName("crm自动化测试报告");
...
//                }
                if (name.length() > 0) {
                    ...
                    name = result.getTestClass().getXmlTest().getName() +":"+ result.getMethod().getMethodName();
                }
                if (extenttest == null) {
